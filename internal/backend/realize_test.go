...
func TestRealize(t *testing.T) {
	...
	customTests := make(sets.Set[string])
	customTest := func(name string, f func(t *testing.T)) {
		t.Helper()
		customTests.Add(name)
		t.Run(name, f)
	}

	customTest("InputReference", func(t *testing.T) {
		t.Parallel()
	...
	customTest("Cores", func(t *testing.T) {
		t.Parallel()
		...
		for _, n := range tests {
			t.Run(fmt.Sprintf("N%d", n), func(t *testing.T) {
				...
				data, err := readTestData(dir, "TestRealize/Cores.txt", nil)
				if err != nil {
					t.Fatal(err)
				}
				if err := data.writeTo(ctx, server, nil); err != nil {
	...
	customTest("Upload", func(t *testing.T) {
		t.Parallel()
	...
	// Now read tests that don't require custom setup from the TestRealize directory.
	...
	for _, entry := range listing {
		...
		if !isTXTAR || customTests.Has(testName) {
			continue
		}
