...
// FixedOutputPath returns an error if [Outputs.IsFixed] reports false.
...
func (drv *Derivation) MarshalText() ([]byte, error) {
	...
	outputs, err := drv.Outputs.all(drv.Dir, drv.Name)
	if err != nil {
		return nil, fmt.Errorf("marshal %s derivation: outputs: %v", drv.Name, err)
	}
...
func (drv *Derivation) parseTuple(s *aterm.Scanner) error {
	...
	for {
		...
		var output Output
		if err := output.parse(s); err != nil {
			return fmt.Errorf("outputs: %v", err)
		}
		if output.IsFloating() {
			newOutputs.names = append(newOutputs.names, output.Name)
			continue
		}

		if newOutputs.fixed != nil {
			return fmt.Errorf("outputs: cannot have more that one fixed output (have %s and %s)",
				DefaultOutputName, output.Name)
		}
		if output.Name != DefaultOutputName {
			return fmt.Errorf("outputs: %s: fixed output must be named %s", output.Name, DefaultOutputName)
		}
		newOutputs.fixed = new(output.ContentAddress)

		if output.Path != "" {
			if drv.Dir == "" {
				drv.Dir = output.Path.Dir()
			} else if output.Path.Dir() != drv.Dir {
				return fmt.Errorf("outputs: %s: %s not in directory %s", output.Name, output.Path, drv.Dir)
			}
			gotName, err := inferDerivationName(output.Path, output.Name)
			if err != nil {
				return fmt.Errorf("outputs: %s: path: %v", output.Name, err)
			}
			if drv.Name == "" {
				drv.Name = gotName
			} else if gotName != drv.Name {
				return fmt.Errorf("outputs: %s: path: %s cannot be used for %s",
					output.Name, output.Path, drv.Name)
			}
			wantPath, err := FixedCAOutputPath(drv.Dir, drv.Name, output.ContentAddress, References{})
			if err != nil {
				return fmt.Errorf("outputs: %s: %v", output.Name, err)
			}
			if output.Path != wantPath {
				return fmt.Errorf("outputs: %s: path: %s should be %s", output.Name, output.Path, wantPath)
			}
		}
	}
